...
function CategoriesScreen() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const limit = productsPerPage;
      const response = await axios.get(
        `https://api.escuelajs.co/api/v1/products/?price_min=${minPrice}&price_max=${maxPrice}&categoryId=${categoryId}&offset=${offset}&limit=${limit}`
      );
  ...
  // Define categories
  const categories = ["Clothing", "Accessories", "Electronics", "Books"]; // Add your own categories here
  ...
  return (
    <div style={{ marginLeft: "10px", marginRight: "10px", backgroundColor: "#E9E6DF" }}>
      ...
      {loading ? (
        ...
        >
          {currentProducts
            .filter((product) => !selectedCategory || product.category === selectedCategory) // Filter products by selected category
            .map((product) => (
              <div
                key={product.id}
